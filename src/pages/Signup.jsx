...
function validatePassword(password, confirmPassword) {
  ...
  if (!passwordRegex.test(password)) {
    toast.error(
      "Password error: Length must be 8 with 1 uppercase 1 lowercase and 1 special character"
    );
  ...
  return (
    <div>
      <form
        ...
        className="flex flex-col justify-center items-center gap-3 text-san-marino"
      >
        {step === 1 && (
          <Step1
            formData={formData}
        ...
        {step === 2 ? (
          <>
            {loading ? (
              ...
              >
                <button
                  type="submit"
                  className="w-28 h-10 rounded-full bg-lime-green hover:border-dashed hover:border-2 hover:border-green-300"
                >
                  Sign Up
                </button>
                <button
          ...
          <div
            id="buttonAndLinks"
            className="w-4/5 sm:w-1/4 flex flex-col items-center justify-center pl-12 pr-12 pt-6"
          >
            <div className="grid justify-items-end w-full">
              <button
                className="flex flex-row text-base pl-8 pt-2 pb-2 gap-2 w-24 h-10 rounded-full bg-lime-green hover:border-dashed hover:border-2 hover:border-green-300 "
                onClick={handleNext}
              >
                Next <img src={SmallRightArrow} alt="" className="h-6" />{" "}
              </button>
            </div>
...
function Step1({ formData, handleChange, setStep, navigate }) {
  return (
    <div
      id="formWrapper"
      className="h-9/12 font-playfair-display text-san-marino pt-11"
    >
      <div
        id="formContainer"
        className="h-4/5 flex flex-col items-center justify-center gap-14"
      >
        ...
        <div
          id="inputContainer"
          className="h-1/4 w-full flex flex-col items-center justify-center gap-9"
        >
          <div class="relative">
            ...
            <input
              type="text"
              class=" text-sm text-black rounded-full w-full pl-10 pr-20 p-2 bg-yellow-tick border border-san-marino 
            placeholder-san-marino hover:border-1 hover:border-dotted"
              placeholder="Name"
              name="name"
              value={formData.name}
          ...
          <div class="relative">
            ...
            <input
              type="number"
              class="text-sm text-black rounded-full w-full pl-10 pr-20 p-2 bg-yellow-tick border border-san-marino
            placeholder-san-marino hover:border-1 hover:border-dotted [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
...
function Step2({ formData, handleChange, setStep, navigate }) {
  return (
    <div
      id="formWrapper"
      className="h-9/12 font-playfair-display text-san-marino pt-12"
    >
      <div
        id="formContainer"
        className="h-4/5 flex flex-col items-center justify-center gap-16"
      >
        <div
          id="formTitleContainer"
          className="w-11/12 flex flex-row text font-extrabold text-2xl sm:gap-0"
        >
